...
import com.agh.io.Sensor.{Sensor, SensorScan}
...
/**
  ...
  */
class PositionCalculator(idx: Integer, configuration: Configuration, map: Map, sensor: Sensor) {
    val sensorScan = sensor.scans(idx)
...
//            }
    ...
    def run(): RatedPosition = {
        val clusters = runClasterization()
        val result = runAnnealing(clusters)
        pool.shutdownNow()
        result
    }

    private def runClasterization(): Seq[Cluster] = {
